...
async function initFirebase() {
    try {
        const { auth: firebaseAuth, db: firebaseDb } = await import('./firebase-config.js');
            ...
            if (user) {
                ...
                try {
                    ...
                    if (userDoc.exists()) {
                        ...
                        // Pre-llenar formulario en paso 2
        ...
        console.log('Firebase no disponible:', error);
    }
}

// Pre-llenar datos del usuario
function prefillUserData(userData, user) {
    // Esperar a que se muestre el paso 2
    const checkStep2 = setInterval(() => {
        const nameInput = document.getElementById('clientName');
        if (nameInput && currentStep === 2) {
            clearInterval(checkStep2);
            
            document.getElementById('clientName').value = `${userData.nombre || ''} ${userData.apellido || ''}`.trim();
            document.getElementById('clientEmail').value = userData.email || user.email;
            document.getElementById('clientPhone').value = userData.telefono || '';
...
function updateLoginButton(user) {
    ...
    if (loginBtn) {
        if (user) {
            ...
            loginBtn.href = 'dashboard-cliente.html';
        } else {
    ...
    if (db && auth) {
        try {
            ...
            const bookingDoc = {
                ...
                apartmentType: bookingData.apartmentType,
            ...
            if (currentUser) {
                console.log('✅ Reserva asociada al usuario:', currentUser.email);
            } else {
                console.log('✅ Reserva guardada como invitado');
            }
...
function showConfirmationMessage() {
    ...
    const messages = {
        'es': userLoggedIn ? 
            `¡Reserva confirmada! Puedes ver el estado de tu reserva en tu panel. Te contactaremos pronto para confirmar tu fecha.` :
            `¡Reserva confirmada! Hemos enviado los detalles por WhatsApp. Te contactaremos pronto para confirmar tu fecha.`,
        'en': userLoggedIn ?
            `Booking confirmed! You can check your booking status in your panel. We will contact you soon to confirm your date.` :
            `Booking confirmed! We have sent the details via WhatsApp. We will contact you soon to confirm your date.`
    };
